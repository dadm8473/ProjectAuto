...
test('service worker keeps the installable mobile game shell available after first load', async () => {
  ...
  for (const marker of [
    "const CACHE_NAME = 'projectauto-reboot-shell-v35';",
    ...
    '/src/client/styles.css?v=lobby-next-title1',
...
test('browser QA verifies the runtime service worker cache activation', async () => {
  ...
  for (const marker of [
    ...
    "cacheName === 'projectauto-reboot-shell-v35'",
    "await cache.match('/index.html')",
    "await cache.match('/src/client/styles.css?v=lobby-next-title1')",
