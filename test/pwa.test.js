...
test('service worker keeps the installable mobile game shell available after first load', async () => {
  ...
  for (const marker of [
    "const CACHE_NAME = 'projectauto-reboot-shell-v48';",
...
test('browser QA verifies the runtime service worker cache activation', async () => {
  ...
  for (const marker of [
    ...
    "cacheName === 'projectauto-reboot-shell-v48'",
    "await cache.match('/index.html')",
    "await cache.match('/src/client/styles.css?v=coop-briefing1')",
    "await cache.match('/src/client/app.js?v=role-detail1')",
    ...
    "await cache.match('/src/client/reboot_render.js?v=role-detail1')",
