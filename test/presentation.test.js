...
test('client app is split into reboot modules and keeps app.js as bootstrap', async () => {
  ...
  for (const marker of [
    ...
    "from './reboot_screens.js?v=operation-road1'",
...
test('app shell cache-busts the game stylesheet for visual asset updates', async () => {
  ...
  assert.equal(html.includes('<link rel="stylesheet" href="/src/client/styles.css?v=operation-road1">'), true);
  ...
  assert.equal(html.includes('<link rel="stylesheet" href="/src/client/styles.css?v=lobby-profile2">'), false);
  assert.equal(html.includes('<link rel="stylesheet" href="/src/client/styles.css?v=lobby-profile1">'), false);
  assert.equal(html.includes('<link rel="stylesheet" href="/src/client/styles.css?v=meta-station1">'), false);
  ...
  assert.equal(app.includes("from './reboot_screens.js?v=operation-road1'"), true);
...
test('first battle command stage is one imagegen summon pod, not three equal web buttons', async () => {
  ...
  assert.equal(html.includes('/src/client/styles.css?v=operation-road1'), true);
  ...
  assert.equal(html.includes('/src/client/styles.css?v=lobby-profile2'), false);
  assert.equal(html.includes('/src/client/styles.css?v=lobby-profile1'), false);
  assert.equal(html.includes('/src/client/styles.css?v=meta-station1'), false);
...
test('meta showcase copy sits on generated nameplates instead of floating over art', async () => {
  ...
  for (const marker of [
    ...
    '.meta-showcase-copy > *,\n.meta-showcase-stats > *',
    'z-index: 1;',
    '<link rel="stylesheet" href="/src/client/styles.css?v=operation-road1">'
  ]) {
...
test('meta shelf cards use generated shelf nameplates for labels prices and states', async () => {
  const css = await readFile('src/client/styles.css', 'utf8');

  for (const marker of [
    '--meta-command-ribbons: url("/src/client/assets/generated/reboot-meta-command-ribbons.png?v=meta-command-ribbons1")',
    '--meta-shelf-nameplates: url("/src/client/assets/generated/reboot-meta-shelf-nameplates-v1.png?v=meta-shelf-nameplates1")',
    ...
    '.meta-shelf-grid .unit-card .role-pill',
    '.meta-shelf-grid .shop-card .role-pill',
  ...
  const unitRoleBlock = cssRuleBlock(css, '.meta-shelf-grid .unit-card .role-pill');
  for (const marker of [
    'display: inline-flex;',
    'position: absolute;',
    'background-image: var(--meta-shelf-nameplates);',
    ...
    assert.equal(unitRoleBlock.includes(marker), true, marker);
  }

  assert.equal(css.includes('.meta-shelf-grid .unit-card .unit-cost {\n  position: static;'), false);

  const shopShelfPriceBlock = cssRuleBlock(css, '.meta-shelf-grid .shop-card .shop-price');
  for (const marker of [
    'position: static;',
    'justify-self: center;',
    'min-width: 50px;',
...
test('meta shelf tiles use generated status overlays instead of catalog cells', async () => {
  const css = await readFile('src/client/styles.css', 'utf8');
  const collection = buildRebootCollection({ xp: 40, unitLevels: { spark_pin: 1 } });
  ...
  for (const marker of [
    '--meta-item-status-overlays: url("/src/client/assets/generated/reboot-meta-item-status-overlays.png?v=meta-item-status1")',
    'data-tile-state="ready"',
    'data-tile-state="locked"',
    'data-tile-state="owned"',
    'data-tile-state="equipped"',
    '.meta-shelf-grid .unit-card::before,\n.meta-shelf-grid .shop-card::before',
    'background-image: var(--meta-item-status-overlays);',
    '.meta-shelf-grid .unit-card[data-tile-state="ready"]::before,',
    '.meta-shelf-grid .shop-card[data-tile-state="ready"]::before',
    '.meta-shelf-grid .unit-card[data-tile-state="locked"]::before,',
    '.meta-shelf-grid .shop-card[data-tile-state="locked"]::before',
    '.meta-shelf-grid .shop-card[data-tile-state="owned"]::before',
    '.meta-shelf-grid .shop-card[data-tile-state="equipped"]::before'
  ]) {
    assert.equal(`${css}\n${collection}\n${shop}`.includes(marker), true, marker);
  }
...
test('result actions use dedicated generated button frames', async () => {
  ...
  for (const marker of [
    ...
    '<button id="resultLobbyButton" class="result-action-button result-action-secondary"><span>준비실</span></button>',
...
test('combat renderer uses generated VFX atlas for action feedback', async () => {
  ...
  for (const marker of [
    'reboot-combat-vfx.png',
	    ...
	    'reboot-combat-action-surges.png?v=action-surges1',
	    'actionSurges',
	    'function drawCombatActionSurgeSprite',
	    'function drawCombatActionSurges',
...
test('combat renderer keeps a generated first-summon landing beacon through onboarding', async () => {
  const render = await readFile('src/client/reboot_render.js', 'utf8');
  ...
  for (const marker of [
    ...
    'function drawFirstSummonLandingBeacon',
    'if (state.now > FIRST_SUMMON_BEACON_END) return false;',
    'drawFirstSummonLandingBeacon(ctx, state, assets, localBoardId);'
  ]) {
    assert.equal(render.includes(marker), true, marker);
  }
});
...
test('combat renderer uses generated partner assist pings for bot co-op actions', async () => {
  ...
  assert.equal(render.includes("ctx.fillText('봇 자동 지원'"), false);
});
...
test('lobby operation card uses a dedicated generated co-op defense scene', async () => {
  ...
  for (const marker of [
    ...
    'class="operation-progress operation-road" aria-label="${operationRoadAriaLabel(operation, roadNodes)}"',
    'class="operation-progress-node operation-road-node"',
    'data-operation-node="${step === operation.step ? \'active\' : \'idle\'}"',
    'class="operation-progress operation-road"',
    'data-operation-road-state="${roadNode.state}"',
    'class="operation-progress-node operation-road-node"',
    ...
    '.operation-progress',
    '.operation-progress.operation-road',
    '.operation-road-node',
    '.operation-road-node[data-operation-road-state="cleared"]',
    '.operation-road-node[data-operation-road-state="current"]',
    '.operation-road-node[data-operation-road-state="locked"]',
...
test('lobby commander rank uses generated game plates without adding a profile menu', async () => {
  const css = await readFile('src/client/styles.css', 'utf8');
  const screens = await readFile('src/client/reboot_screens.js', 'utf8');

	for (const marker of [
	  'function lobbyProfileModel',
	  'function buildLobbyProfilePlate',
	  ...
	  'class="lobby-profile-plate" aria-label="${profileAriaLabel}"',
	  'class="lobby-profile-emblem" data-rank-medal="${profileRank.medal}"',
    'class="lobby-profile-label"',
    'class="lobby-profile-progress" data-progress-kind="season" style="--profile-progress:${profileRank.progress}%"',
    '.lobby-profile-plate',
    ...
    'clip-path: inset(0 calc(100% - var(--profile-progress)) 0 0);'
  ]) {
    assert.equal(`${css}\n${screens}`.includes(marker), true, marker);
  }

  for (const forbidden of [
    'data-lobby-open="profile"',
    'class="lobby-card profile-hook"',
    'class="screen-card profile-hook"',
    '.lobby-profile-plate {\n  background:'
  ]) {
    assert.equal(`${css}\n${screens}`.includes(forbidden), false, forbidden);
  }
...
test('client wires missions and season pass claim loops', async () => {
  ...
  for (const marker of [
    ...
    'data-mission-claim="${featuredMission.id}"',
    'data-pass-claim="${featuredTier.index}"',
    '>${CLAIM_ACTION_LABEL}</button>',
    'handleMissionClaim',
    'handlePassClaim',
    'claimedMissions',
    'claimedPassTiers',
...
test('meta banner art is not dimmed by css-only rgba overlays', async () => {
  ...
  for (const selector of [
    '.mission-stamp-board::after,\n.season-track-board::after',
    '.meta-showcase::after',
    '.meta-showcase[data-showcase-kind="collection"]::after,\n.meta-showcase[data-showcase-kind="shop"]::after',
    '.meta-showcase[data-showcase-kind="shop"]::after'
  ]) {
    const block = cssRuleBlock(css, selector);
    assert.equal(block.includes('background: transparent;'), true, selector);
    assert.equal(block.includes('background: rgba('), false, selector);
    assert.equal(block.includes('linear-gradient'), false, selector);
    assert.equal(block.includes('backdrop-filter'), false, selector);
  }
});
...
test('collection and shop use a generated display shelf instead of list-only rows', async () => {
  ...
  assert.equal(shelfFrameBlock.includes('background-image: var(--meta-item-status-overlays);'), true);
  assert.equal(shelfFrameBlock.includes('background-size: 400% 100%;'), true);

  const shelfNameplateBlock = cssRuleBlock(css, '.meta-shelf-grid .card-copy strong');
  assert.equal(shelfNameplateBlock.includes('background-image: var(--meta-shelf-nameplates);'), true);
  assert.equal(shelfNameplateBlock.includes('background-size: 400% 100%;'), true);
  assert.equal(shelfNameplateBlock.includes('background-position: 0 0;'), true);
...
test('mission command board fills the center with generated stamp sockets', async () => {
  const css = await readFile('src/client/styles.css', 'utf8');

  const missionBoardBlock = cssRuleBlock(css, '#missionsList .mission-stamp-board');
  for (const marker of [
    'grid-template-columns: clamp(92px, 27vw, 112px) minmax(0, 1fr) clamp(78px, 22vw, 96px);',
    'grid-template-areas:',
    '"copy focus command"',
    '"stamps stamps command"',
    'min-height: clamp(194px, 52vw, 226px);'
  ]) {
    assert.equal(missionBoardBlock.includes(marker), true, marker);
  }

  const stampGridBlock = cssRuleBlock(css, '#missionsList .mission-stamp-grid');
  for (const marker of [
    'grid-column: auto;',
    'align-self: center;',
    'transform: none;',
    ...
    assert.equal(stampGridBlock.includes(marker), true, marker);
  }

  const stampSlotBlock = cssRuleBlock(css, '#missionsList .mission-stamp-slot');
  assert.equal(stampSlotBlock.includes('min-height: clamp(54px, 15vw, 66px);'), true);
});

test('mission command board avoids stamp and command overlap on compact phones', async () => {
  const css = await readFile('src/client/styles.css', 'utf8');
  const compactStart = css.indexOf('@media (max-width: 350px)');
  assert.notEqual(compactStart, -1, 'compact mission board media query is missing');
  const compactCss = css.slice(compactStart);

  const missionBoardBlock = cssRuleBlock(compactCss, '#missionsList .mission-stamp-board');
  for (const marker of [
    'grid-template-columns: minmax(0, 1fr) clamp(98px, 31vw, 104px);',
    '"copy command"',
    '"stamps stamps"'
  ]) {
    assert.equal(missionBoardBlock.includes(marker), true, marker);
  ...
  const stampGridBlock = cssRuleBlock(compactCss, '#missionsList .mission-stamp-grid');
  assert.equal(stampGridBlock.includes('grid-column: auto;'), true);
  assert.equal(stampGridBlock.includes('grid-template-columns: repeat(3, minmax(0, 1fr));'), true);

  const commandBlock = cssRuleBlock(compactCss, '#missionsList .mission-board-command');
  assert.equal(commandBlock.includes('grid-template-columns: 34px minmax(58px, auto);'), true);
  assert.equal(commandBlock.includes('gap: 4px;'), true);

  const featuredActionBlock = cssRuleBlock(compactCss, '#missionsList .featured-objective-action');
  assert.equal(featuredActionBlock.includes('min-width: 58px;'), true);
  assert.equal(featuredActionBlock.includes('min-height: 44px;'), true);
});
...
test('mission and season boards promote the next condition on generated focus plates', async () => {
  const css = await readFile('src/client/styles.css', 'utf8');
  const screens = await readFile('src/client/reboot_screens.js', 'utf8');
  const missions = buildMissionScreen({ processedRuns: [], claimedMissions: [] });
  const season = buildSeasonScreen({ xp: 80, claimedPassTiers: [0] });
  ...
  for (const marker of [
    'function missionFocusModel',
    'function seasonFocusModel',
    'class="objective-focus"',
    'data-focus-kind="mission"',
    'data-focus-kind="season"',
    'data-focus-state="locked"',
    '다음 목표',
    '다음 보상',
    '전투 1회 완료 · 0/1',
    '80/160 XP',
    ...
    'background-image: var(--meta-status-plaques);',
    'background-size: 200% 100%;',
    '.objective-focus[data-focus-kind="season"]',
    '.objective-focus p'
  ]) {
    assert.equal(combined.includes(marker), true, marker);
  }

  const focusBlock = cssRuleBlock(css, '.objective-focus');
  for (const marker of [
    'display: grid;',
    'align-content: center;',
    'min-height: clamp(70px, 20vw, 88px);',
    'background-image: var(--meta-status-plaques);',
    'background-repeat: no-repeat;',
    'background-size: 200% 100%;'
  ]) {
    assert.equal(focusBlock.includes(marker), true, marker);
  }

  const focusTextBlock = cssRuleBlock(css, '.objective-focus span,\n.objective-focus p');
  assert.equal(focusTextBlock.includes('background-image: var(--meta-caption-plate);'), true);
...
test('compact season board narrows its XP counter away from the claim command', async () => {
  const css = await readFile('src/client/styles.css', 'utf8');
  const compactSeasonCounterBlock = cssRuleBlock(css, '#seasonList .season-board-copy strong');

  assert.equal(compactSeasonCounterBlock.includes('width: clamp(104px, 32vw, 124px);'), true);
  assert.equal(compactSeasonCounterBlock.includes('padding: 0 12px;'), true);
});

test('mission and season reward rows avoid duplicate claim buttons below the top command', async () => {
  const css = await readFile('src/client/styles.css', 'utf8');
  const missions = buildMissionScreen({ processedRuns: ['run-1'], claimedMissions: [] });
  const season = buildSeasonScreen({ xp: 80, claimedPassTiers: [] });
...
test('mission and season rows show compact generated progress readouts inside each slot', async () => {
  const css = await readFile('src/client/styles.css', 'utf8');
  const missions = buildMissionScreen({ processedRuns: [], claimedMissions: [] });
  const season = buildSeasonScreen({ xp: 0, claimedPassTiers: [] });
  const combined = `${css}\n${missions}\n${season}`;
  ...
  for (const marker of [
    'class="objective-readout"',
    '0/1',
    '0/60',
    '.meta-progress-board .objective-readout',
    'background-image: var(--meta-caption-plate);',
    'background-size: 100% 100%;'
  ]) {
    assert.equal(combined.includes(marker), true, marker);
  ...
  const copyBlock = cssRuleBlock(css, '.meta-progress-board .card-copy');
  for (const marker of [
    'position: relative;',
    'z-index: 2;',
    'justify-items: center;',
    'grid-template-rows: minmax(30px, auto) minmax(20px, auto) 16px;'
  ]) {
    assert.equal(copyBlock.includes(marker), true, marker);
  }
  ...
  for (const marker of [
    ...
    assert.equal(readoutBlock.includes(marker), true, marker);
  }
...
test('combat HUD values are visually dominant over labels on phone sockets', async () => {
  const css = await readFile('src/client/styles.css', 'utf8');

  const valueBlock = cssRuleBlock(css, '.meter-value');
  assert.equal(valueBlock.includes('font-size: clamp(13px, 3.8vw, 16px);'), true);
  assert.equal(valueBlock.includes('font-weight: 1000;'), true);
  assert.equal(valueBlock.includes('min-width: 20px;'), true);

  const compactStart = css.indexOf('@media (max-width: 360px)');
