...
export function eventLabel(event) {
  ...
  if (event.type === 'boss_mirror_linkbreak') return '보스 미러가 팀 링크 파괴';
  if (event.type === 'boss_origin_spore') return `기원 널 포자 ${event.spawnedSpores?.length ?? 0}개 생성`;
  if (event.type === 'link_pulse_save') return '파트너 펄스로 보드 구원';
  if (event.type === 'link_pulse') return '파트너 펄스로 릴레이 냉각';
