...
function drawCombatStartCutin(ctx, state, assets = {}, operationMeta = {}) {
  ...
  ctx.save();
  drawImageCover(ctx, image, 0, 164, 390, 112, alpha);
  drawAtlasSprite(ctx, assets, 'ui', 'summon_charge', 76, 223, 44, alpha);
  ctx.globalAlpha *= alpha;
  ctx.fillStyle = '#fff7dc';
  ...
  ctx.strokeText?.(operation.title, 118, 214);
  ctx.fillText(operation.title, 118, 214);
  ctx.shadowBlur = 0;
  ctx.fillStyle = 'rgba(245, 240, 220, 0.86)';
  ctx.font = '850 12px system-ui';
  ctx.fillText(operation.subtitle, 118, 236);
  ctx.restore();
  return true;
}
